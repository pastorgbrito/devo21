...
export default function ObservationChatPage({ params }: { params: Promise<{ id: string, dayNum: string }> }) {
    ...
    // Simulación rápida de un chat para ver el diseño (Se conectará a Gemini en Fase 4)
    const [messages] = useState([
        { id: 1, sender: 'ia', text: "¿Cómo crees que la práctica del silencio esta mañana ha cambiado tu perspectiva sobre el estrés cotidiano? ✨" },
        { id: 2, sender: 'user', text: "Me sentí más en paz, aunque mi mente divagaba a ratos hacia las tareas pendientes." },
    ]);
    ...
    const handleContinue = () => {
        // Al terminar el debate socrático nos vamos a Interpretación
        router.push(`/study/${resolvedParams.id}/day/${resolvedParams.dayNum}/interpretation`);
    };
    ...
    return (
        <div className="min-h-screen bg-[#070B19] text-white flex flex-col font-sans">
            ...
            <header className="sticky top-0 z-50 bg-[#070B19]/80 backdrop-blur-xl border-b border-white/5 py-4 px-4 flex items-center gap-4">
                <button
                    onClick={() => router.push(`/study/${resolvedParams.id}/day/${resolvedParams.dayNum}`)}
                ...
                <div className="flex-1 flex items-center gap-3 bg-[#131B2F] border border-white/5 rounded-2xl p-2 pr-4">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] text-slate-400 tracking-wide">{study.cabecera.titulo} • Observación</p>
                    </div>
                </div>
            ...
            <div className="fixed bottom-20 left-1/2 -translate-x-1/2 w-full max-w-md bg-gradient-to-t from-[#070B19] via-[#070B19] to-transparent pt-10 pb-6 px-4">
                <div className="w-full flex items-end gap-2 bg-[#0A0D18] border border-white/10 rounded-[1.5rem] p-2 pr-2 shadow-2xl">
                    ...
                    <textarea
                        ...
                        placeholder="Responde desde el corazón..."
